...
import { PROMPT_ROTEADOR, PROMPT_CARDIOLOGIA, PROMPT_DERMATOLOGIA, PROMPT_ENDOCRINOLOGIA, PROMPT_GERAL } from './prompts.js';
export async function processarMensagemLLM(sessao, mensagemUsuario) {
    ...
    if (agenteAtual === 'orquestrador' || !agenteAtual) {
        const respostaRoteador = await chamarLLMComCascata(PROMPT_ROTEADOR, [{ role: 'user', content: mensagemUsuario }]);
        try {
    ...
    const historicoFormatado = historicoBruto.map((msg) => ({
    ...
    const contextoRAG = await buscarContexto(mensagemUsuario);
    if (contextoRAG) {
